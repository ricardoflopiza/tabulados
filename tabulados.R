...
plan2021 = read.xlsx("plan_tabulado2021.xlsx")
plan2019 = read.xlsx("plan_tabulado.xlsx")
llave = read.xlsx("/home/ricardo/Documents/INE/ENUSC/Enusc2021/bases/nombres_estandar_enusc.xlsx")  

plan2021$Cuadro = stringr::str_remove(tolower(plan2021$Cuadro),".$")
plan2019$Cuadro = stringr::str_remove(tolower(plan2019$Cuadro),".$")
plan2021$Nombre.variable = stringr::str_remove(plan2021$Nombre.variable,".$")

plan2021 = plan2021 %>% left_join(llave[,c("Nombre.FULL.2021.Reducida","Nombre.FULL.2019")],by = c("Nombre.variable" =  "Nombre.FULL.2021.Reducida")) %>% 
  left_join(plan2019[,c(5:8)], by = c("Nombre.FULL.2019"= "Nombre.variable")) %>%  select(- Nombre.FULL.2019, -Tipo.de.indicador)

plan2021$filtro[plan2021$Nombre.variable == "J5_1_1"] = "F1_1_1 ==1"
...
for(i in 1:55){ variable = plan2021$Nombre.variable[i]  
print(attr(enusc2021[[variable]],"labels")) 
}

enusc2021[[plan2021$Nombre.variable[5]]]

#### pasamos a dummyes según especicaciones de variables
for(i in 1:55){
  
  variable =  plan2021$Nombre.variable[i] 
  ...
  print(table(enusc2021[,plan2021$Nombre.variable[i]]))
}
...
tictoc::tic()
for (i in c(1:55)) {
#variable =  paste0(plan2021$Nombre.variable[i],"_dum")
variable =  plan2021$Nombre.variable[i]
  
  subpopOBJ = NULL
if(plan2021$filtro[i] != "no"){
  subpopOBJ = plan2021$filtro[i]
  
  subpopOBJ = stringr::str_remove_all(subpopOBJ," ==1| == 1")
  
}

  if(plan2021$Factor.de.expansión[i] == "Persona"){  
  ...
  tabulado[2,1] = plan2021$Cuadro[i]
  tabulado[3,1] = plan2021$Nombre.variable[i]
...
names(tablas) = plan2021$Nombre.variable[1:55]
...
for (i in 1:55) {
  ...
  if(plan2021$Factor.de.expansión[i] != "Persona"){
    ...
    pest <- as.character(plan2021$Nombre.variable[i])
...
plan2021[,c("Cuadro",  "Desagregación")] -> ind
